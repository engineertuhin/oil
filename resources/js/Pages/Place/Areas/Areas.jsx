...
export default function Areas({ auth, getData, districs }) {
    const toast = useRef(null);
    const [model, setModel] = useState(false);
    const [datas, setData] = useState(getData);
    const [districsList, setDistrics] = useState(districs);
    ...
    const dataDelete = (id) => {
        ...
        axios.delete(route("areas.destroy", id)).then((res) => {
    ...
    // Insert or Update
    const onSubmit = (data) => {
        axios.post(route("areas.store"), data).then((res) => {
            setData(res.data.data);
            if (!data.id) {
                reset({
                    name: "",
    ...
    return (
        <AuthenticatedLayout
            ...
            header={
                <>
                    <h2 className="text-skin-header font-medium ">Areas</h2>{" "}
